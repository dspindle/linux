...
#define _cshift_pad_call(_is_set, _sd, _op, _cfg, _info, _code_)		\
...
{
	return cshift_pad_call(0, sd, enum_mbus_code, cfg, code, &code->code);
}
...
{
	return cshift_pad_call(0, sd, enum_frame_size, cfg, fse, &fse->code);
}
...
{
	return cshift_pad_call(0, sd, get_fmt, cfg, fmt, &fmt->format.code);
}
...
static int cshift_pad_set_fmt(struct v4l2_subdev *sd,
			      ...
			      struct v4l2_subdev_format *fmt)
{
	return cshift_pad_call(1, sd, set_fmt, cfg, fmt, &fmt->format.code);
}
