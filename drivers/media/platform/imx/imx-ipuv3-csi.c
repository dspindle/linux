...
int ipucsi_resume_stream(struct ipucsi *ipucsi)
{
	...
	eba = vb2_dma_contig_plane_dma_addr(&vb->vb2_buf, 0);
	if (ipucsi->ilo < 0)
		eba -= ipucsi->ilo;
				   ...
				   unsigned int notification, void *arg)
{
	...
	if (notification == V4L2_SUBDEV_SYNC_LOCK_NOTIFY) {
		ret = media_entity_graph_walk_init(&graph, sd->v4l2_dev->mdev);
		if (ret)
		{
			return;
		}
...
static bool ipucsi_finish_frame(struct ipucsi *ipucsi)
{
	...
	vb = &vbuf->vb2_buf;
	buf = container_of(vbuf, struct ipucsi_buffer, vb);
		...
		unsigned int sizes[], void *alloc_ctxs[])
{
	...
	struct v4l2_format *fmt = &ipucsi->format;
...
static void ipucsi_videobuf_queue(struct vb2_buffer *vb)
{
	...
	struct vb2_queue *vq = vb->vb2_queue;
	struct ipucsi *ipucsi = vq->drv_priv;
	struct ipucsi_buffer *buf = container_of(vbuf, struct ipucsi_buffer, vb);
	unsigned long flags;
...
int v4l2_media_subdev_s_stream(struct media_entity *entity, int enable)
{
	...
	sd = media_entity_to_v4l2_subdev(first);
	ret = media_entity_graph_walk_init(&graph, sd->v4l2_dev->mdev);
	if (ret)
	{
		return ret;
...
static int ipucsi_subdev_init(struct ipucsi *ipucsi, struct device_node *node)
{
	...
	ret = media_entity_pads_init(&ipucsi->subdev.entity, 2, ipucsi->subdev_pad);
	if (ret < 0)
		return ret;
...
static int ipucsi_probe(struct platform_device *pdev)
{
	...
	ret = media_create_pad_link(&ipucsi->subdev.entity, 1,
...
failed:
	...
	if (!list_empty(&ipucsi->vdev.entity.links))
