...
static void ipu_complete(void *priv, int err)
{
	...
	unsigned long flags;
	struct vb2_v4l2_buffer *dst_vbuf, *in_vbuf;
	...
	in_vbuf = to_vb2_v4l2_buffer(ctx->in);
	dst_vbuf->timecode = in_vbuf->timecode;
...
static void device_run(void *priv)
{
	...
	if (pix->field == V4L2_FIELD_INTERLACED_BT) {
		...
		image_in[0].phys0 = vb2_dma_contig_plane_dma_addr(ctx->in_p, 0);
		image_in[1].phys0 = vb2_dma_contig_plane_dma_addr(ctx->in, 0) +
			pix->bytesperline;
		image_in[2].phys0 = vb2_dma_contig_plane_dma_addr(ctx->in, 0);
	...
	image_out.phys0 = vb2_dma_contig_plane_dma_addr(dst_buf, 0);
