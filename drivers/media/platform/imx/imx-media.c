...
*/
	v4l2_info(link->sd, "registered sensor subdev '%s'\n", sd->name);
...
static int ipu_media_complete(struct v4l2_async_notifier *notifier)
{
	...
	struct ipu_media_controller *ctl = container_of(notifier->v4l2_dev,
						struct ipu_media_controller,
						v4l2_dev);
...
/*
	ret = media_create_pad_link(&sd->entity, portno, &link->sd->entity,
				    link->padno, link->media_link_flags);
*/
	ret = media_create_pad_link(&link->sensor_sd->entity, portno, &link->sd->entity,
				    link->padno, link->media_link_flags);
	if (ret)
		return ret;
...
*/
	...
	v =  atomic_dec_return(&ctl->num_links);

	WARN_ON(link->completed);
	link->completed = true;

	/* HACK: this code is ugly but there does not exist a function to
	 * register a single subdev node.  So, we have to count the number of
	 * all links and generate subdevs when all have been linked */
	if (WARN_ON(v < 0))
		return -EINVAL;
	else if (v > 0)
		return 0;
	else {
		/* ideally, we would register [notifier]->sd only */
		ret = v4l2_device_register_subdev_nodes(&ctl->v4l2_dev);
		if (ret < 0)
			return ret;

		return media_device_register(&ctl->mdev);
	}
}
...
void ipu_media_entity_remove_link(struct ipu_media_link *link)
{
	struct ipu_media_controller	*ctl =
		container_of(link->asn.v4l2_dev, struct ipu_media_controller,
			     v4l2_dev);

	if (link->completed)
		atomic_inc(&ctl->num_links);
...
int ipu_media_device_register(struct device *dev, unsigned int num_links)
{
	...
	atomic_set(&ipu_media->num_links, num_links);
