...
static void ipu_scaler_work(struct work_struct *work)
{
	...
	dst_vbuf = to_vb2_v4l2_buffer(dst_buf);
	src_vbuf = to_vb2_v4l2_buffer(src_buf);
	dst_vbuf->timecode = src_vbuf->timecode;

	spin_lock_irqsave(&ipu_scaler->irqlock, flags);
	v4l2_m2m_buf_done(src_vbuf, err ? VB2_BUF_STATE_ERROR :
					 VB2_BUF_STATE_DONE);
	v4l2_m2m_buf_done(dst_vbuf, err ? VB2_BUF_STATE_ERROR :
					 VB2_BUF_STATE_DONE);
